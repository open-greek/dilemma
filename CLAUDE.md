...
Multi-source dedup: each work is counted once in `total`/`by_*` by source priority (glaux > diorisis > first1k > pta > byz), keyed by TLG id; lower-priority sources of a claimed work contribute only `source_counts` + citations. TLG-bearing TEI works inherit GLAUx's century/genre/dialect; others use a per-source genre bucket (first1k=other, pta=religion, byz=poetry) plus a composition century from the PTA `<creation>` header or the byzantine `manifest.json`. `--sources` selects a subset; canonical-greekLit (classical, ~fully overlapping GLAUx/Diorisis) and PG (`.vert`, not on disk) are not in the default set. Memory is bounded by streaming citations to a temp SQLite `_raw` table (the `CiteSink`), not a Python list.

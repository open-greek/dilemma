...
no torch, no onnxruntime, no network. Mirrors `dilemma.morph_diff`'s
shape. The optional `attested_only` / `with_attestation` flags make
read-only stdlib `sqlite3` queries against the form-attestation DBs
(`dilemma._attest_db`); they stay off by default, so the default path
touches no DB.
...
class ParadigmForm:
    ...
    # Corpus attestation of the form, populated only when generate(...) is
    # called with with_attestation=True; None otherwise. compare=False keeps
    # equality/hashing on (form, source) and out of the citation blob.
    attestation: Optional[dict] = field(default=None, compare=False)
...
_ATTEST_DB = None  # lazy dilemma._attest_db.AttestDB, for the attestation flags
...
def reset_cache() -> None:
    ...
    """
    global _SOURCE_CACHE, _ATTEST_DB
    _SOURCE_CACHE = None
    _ATTEST_DB = None


def _attest_db():
    """Lazily open the shared form-attestation DBs (pure-stdlib sqlite3)."""
    global _ATTEST_DB
    if _ATTEST_DB is None:
...
def generate(
    ...
    *,
    ...
    `attested_only=True` filters the precedence chain to the first source
    whose form is attested in the corpus, rather than picking the top source
    and then dropping it: a higher-precedence form that is unattested falls
    through to a lower-precedence form that is. Matching is grave/case-folded
    (generated forms are citation-style, so they would never match the corpus's
    grave-accented or sentence-capitalized surface forms exactly). Returns None
    if no source (templates included) yields an attested form.

    `with_attestation=True` sets the returned form's `attestation` to the corpus
    record of the matched surface form (`max_citations` bounds the example loci).
    Both flags need `form_profile.db` (download with `python -m dilemma download`).
    ...
    db = _attest_db() if (attested_only or with_attestation) else None

    def _make(form, src):
        att = None
        if with_attestation:
            att = db.attestation_by_norm(form, max_citations=max_citations)
        return ParadigmForm(form=form, source=src, attestation=att)

    # Candidate forms in precedence order (sources, then template fallback).
    candidates = []
    ...
    if allow_template:
        ...
        if f:
            candidates.append((f, ParadigmSource.TEMPLATE.value))

    for form, src in candidates:
        if attested_only and not db.is_attested_norm(form):
            continue  # fall through to a lower-precedence attested form
        return _make(form, src)
    return None
...
def generate_paradigm(
    ...
    *,
    ...
    are simply absent from the dict; with `attested_only=True`, cells whose
    form is unattested in the corpus are likewise absent. See `generate` for
    the attestation flags.
    """
    if lang != "grc":
        return {}
    ...
    for slot in iter_slots(pos):
        f = generate(
            lemma, slot, lang=lang, allow_template=allow_template,
            attested_only=attested_only, with_attestation=with_attestation,
...
def fill_canonical_dict(
    ...
    *,
    ...
    This is intentionally NOT wired to `generate`'s `attested_only` /
    `with_attestation` flags: the downstream canonical (Klisy) build wants full
    paradigm coverage, including correct-but-unattested cells, so gating here
    would silently drop cells it expects.
